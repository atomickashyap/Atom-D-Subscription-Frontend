          ...
          subscriptions.map((sub) => (
            <div key={sub.id}>
              <section>
                ...
                <button className="btn btn-outline-danger" onClick={() => {history.push(`/${sub.plan.nickname.toLowerCase()}`)}}>
                    Access
                </button>{" "}
                <button className="btn btn-outline-warning" onClick={manageSubscription}>
                  Manage Subscription
                </button>
              </section>
            </div>
